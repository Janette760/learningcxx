...
int main(int argc, char **argv) {
    ...
    }
    {
        ...
        ref = false;
        ASSERT(!ref, "Fill in `ref` or `!ref`");
        // THINK: WHAT and WHY?operator[] 返回的不是普通的 bool& 类型，而是一个代理对象（proxy object）。这是因为 std::vector<bool> 采用了位压缩技术，
        //每个 bool 值可能只占一个位，无法直接返回对单个位的引用，所以返回一个代理对象来模拟引用的行为。
        //由于 ref 是代理对象，这个赋值操作会修改 vec[30] 对应的位的值。
